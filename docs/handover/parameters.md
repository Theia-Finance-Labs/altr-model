...
| `dynamic_marginal_ef` | `True` | Carbon cost | Let the marginal emission factor decay with the VRE capacity share,<br>marginal_ef(t) = marginal_EF × (1 − vre_share(t))², so a technology loses its<br>carbon rent as renewables push it off the margin.<br><br>Both keys act on marginal_EF, which the market-clearing-price adjustment<br>produced. That adjustment is retired here (2026-09-01 owner ruling), so<br>marginal_EF is 0, the two methods coincide, and this key is inert. Kept so the<br>differential path works if the adjustment ever returns. |
...
| `replacement_capex_rate` | `0.02` | Cost switches | Fraction (0.0–1.0) of a real asset's INSTALLED capacity capitalized each year as<br>roll-over / replacement CapEx — routine capital maintenance and refurbishment,<br>for which 1–3% of replacement cost per year is the standard utility benchmark<br>(EPRI; Lazard LCOE methodology). Read only when include_replacement_capex is<br>True; charged on `asset_trajectory`, so a flat asset still pays it, and skipped<br>for assets retiring that year (already charged decommissioning) and for<br>synthetic assets (accounting constructs, not plant). |
...
| `ownership_type` | `"direct"` | Ownership tier | Which rung of the ownership tree attaches assets to companies.<br>Values: "direct" (ownership_type == "direct", or ownership_level == 1)<br>\| "indirect" (ownership_type == "indirect", or ownership_level >= 2).<br>A company's stake in an asset is recorded at several tiers — a direct holding<br>and the equity stakes rolling up through subsidiaries. They are alternative<br>views of the same capacity, not additive ones, so a run reports on one tier.<br>The tier is selected BEFORE stakes are consolidated, so consolidation only ever<br>sums within the tier it was given.<br>Read only when `ownership_aggregation` is "tier_filter" (the default); "sum"<br>keeps every tier and ignores this key. |
