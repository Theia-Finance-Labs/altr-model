...
| `ownership_type` | `"direct"` | Scope filters | Ownership level used to attach assets to companies.<br>Values: "direct" (ownership_level == 1) \| "indirect" (ownership_level >= 2).<br>The tier is selected first; within it, every stake a company holds in the same asset-year is then totalled into one row. Selecting a tier that does not partition the asset still over-allocates capacity — totalling merges rows, it does not rescale them. |
