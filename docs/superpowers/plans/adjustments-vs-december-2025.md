...
| --- | --- | --- | --- | --- | --- |
| 1 | **Replacement CapEx** (`include_replacement_capex`) | Off — no capital-maintenance charge (DEC-CONFIRM; matches Bertrand's 2026 main default) | On: 2%/yr on installed capacity, retiring/synthetic assets excluded | Headline **−88.3 bn (signal +2.2%)**. Levels: baseline −2,713.9 bn (from +2.3 tn to −0.4 tn), shock −2,802.2 bn. 91.8% of companies move >10%. Lands on renewables (Solar, Wind), fossil nearly untouched | **DECIDED** (Jakub, 2026-09-04): **dropped and deleted from the code**. IAM O&M already bundles capital upkeep — the reasoning that keeps growth CapEx off — so the 2%/yr charge double-counted; a per-technology rate was considered and rejected. Measured on the candidate (R12): switching it off moves the signal -207.3 bn (-6.1%, the signal *grows*) and Σ baseline NPV −305 → +2,462 bn; the ruled configuration with decom at 15% (R13) sits at -3,327 bn vs R1's −3,409, with 57.8% of companies still baseline-negative (fossil-dominant, operating-level). Deletion verified equivalent (R14 ≡ R13). See `measurement-batch-results.md` R12–R14 |
