...
import pandas as pd
import pytest
from crispy_kedro.pipelines.reporting.nodes import build_reporting_views
...
def test_npv_decomp_one_row_per_owner_with_own_pv_revenue():
    ...
    for asset_id, company_id, scale in OWNERS:
        ...
        assert indexed.loc[(asset_id, company_id), "PV_Revenue"] == _approx(
            scale * 6
        )
        assert indexed.loc[(asset_id, company_id), "PV_VarCost"] == _approx(
            scale * 0.3
        )
