...
An asset that appears in two geographies was grouped only by
...
(and a phantom retirement) at each geography's first year.
"""
import pandas as pd
import pytest

from crispy_kedro.pipelines.earnings_model.nodes import compute_capacity_flows


def test_missing_scenario_geography_raises_valueerror():
    panel = pd.DataFrame(
        {
