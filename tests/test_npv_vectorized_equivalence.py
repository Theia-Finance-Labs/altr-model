...
tested dropna fix (see tests/test_npv_dropna_keys.py); folding it into the
reference isolates this file to the vectorisation change alone.

Every branch of the terminal-value ladder is exercised: stranded, finite
annuity, perpetuity, r <= g, final_fcff == 0, negative final FCFF with
stranding off, single-row groups, normalisation windows 1 and 3,
...
# Synthetic fixture
# --------------------------------------------------------------------------
def _asset(asset_id, alignment, years_fcff, trajectory="baseline",
           scenario_type="baseline", company_name=None):
    ...
    return [
        {
            ...
            "company_name": company_name if company_name is not None else f"co_{asset_id}",
...
def _fixture_frame() -> pd.DataFrame:
    rows = []
    # A1 profitable carbontech, multi-year -> finite annuity when stranding on
    rows += _asset("A1", "misaligned_high_carbon",
                   [(2025, 100.0), (2026, 110.0), (2027, 120.0),
                    (2028, 130.0), (2029, 140.0), (2030, 150.0)])
    # A1 shock trajectory (non-baseline scenario_type -> shock discount rate)
    rows += _asset("A1", "misaligned_high_carbon",
                   [(2025, 100.0), (2026, 90.0), (2027, 70.0),
                    (2028, 40.0), (2029, 10.0), (2030, -5.0)],
                   trajectory="latesudden", scenario_type="AR6_shock")
    # A2 stranded carbontech: 3 trailing losses
    rows += _asset("A2", "aligned_high_carbon",
                   [(2025, 80.0), (2026, 40.0), (2027, 5.0),
                    (2028, -10.0), (2029, -20.0), (2030, -30.0)])
    # A3 greentech, growing -> perpetuity
    rows += _asset("A3", "misaligned_low_carbon",
                   [(2025, 50.0), (2026, 60.0), (2027, 70.0),
                    (2028, 80.0), (2029, 90.0), (2030, 100.0)])
    # A4 greentech all-negative: negative final_fcff, perpetuity with negative CF
    rows += _asset("A4", "aligned_low_carbon",
                   [(2025, -10.0), (2026, -20.0), (2027, -30.0),
                    (2028, -40.0), (2029, -50.0), (2030, -60.0)])
    # A5 single-row group (window / n_check clamping)
    rows += _asset("A5", "misaligned_high_carbon", [(2030, 250.0)])
    # A6 NaN group key (company_name)
    rows += _asset("A6", "misaligned_low_carbon",
                   [(2025, 30.0), (2026, 35.0), (2027, 40.0)],
                   company_name=np.nan)
    # A7 final FCFF exactly zero -> no terminal value
    rows += _asset("A7", "aligned_high_carbon",
                   [(2025, 90.0), (2026, 60.0), (2027, 0.0)])
    # A8 flow-split duplicate years (must collapse before anchoring)
    rows += _asset("A8", "misaligned_high_carbon",
                   [(2028, 60.0), (2029, 65.0), (2030, 100.0), (2030, -30.0)])
    # A9 two-row group with mixed sign trailing values
    rows += _asset("A9", "misaligned_high_carbon", [(2029, -5.0), (2030, 12.0)])
    # A10 rows with missing scenario_type -> dropped by both implementations
    rows += _asset("A10", "misaligned_low_carbon",
                   [(2025, 10.0), (2026, 20.0)], scenario_type=np.nan)
...
def test_terminal_rows_are_added_for_the_expected_groups():
    ...
    out = compute_yearly_npv_trajectories(_fixture_frame(), stranding_aware_tv=True)
    tv = out.loc[pd.to_numeric(out["terminal_value"], errors="coerce") != 0]
    assert set(tv["asset_id"]) >= {"A1", "A3", "A5", "A8"}
    # A2 is stranded, A7 has final_fcff == 0 -> no terminal row
    assert "A2" not in set(tv.loc[tv["trajectory_type"] == "baseline", "asset_id"])
...
_REAL = os.path.join(
    ...
    "data", "07_model_output", "asset_earnings.csv",
)
...
def test_real_data_sample_equivalence(capsys):
    ...
    ref = _reference_compute(sample, stranding_aware_tv=True,
                             terminal_normalization_window=3)
    t_ref = time.perf_counter() - t0

    t0 = time.perf_counter()
    new = compute_yearly_npv_trajectories(sample, stranding_aware_tv=True,
                                          terminal_normalization_window=3)
    ...
    ref_g = (_norm(ref).groupby(GROUP_KEYS, dropna=False)[["yearly_npv", "terminal_value"]]
             .sum().sort_index())
    new_g = (_norm(new).groupby(GROUP_KEYS, dropna=False)[["yearly_npv", "terminal_value"]]
             .sum().sort_index())
    assert list(ref_g.index) == list(new_g.index)
    np.testing.assert_allclose(
        new_g.to_numpy(), ref_g.to_numpy(), rtol=1e-6, atol=1e-6
    )
