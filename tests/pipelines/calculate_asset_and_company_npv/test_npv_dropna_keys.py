...
The trajectory loop and the asset-level aggregation used pandas' default
dropna=True, so a row whose company_name (or any other group key) is NaN was
dropped downstream without a trace: present in the input, absent from the
output. All the valuation groupbys must agree.
"""
...
def _frame():
    rows = []
    for traj, stype in [("baseline", "baseline"), ("latesudden", "target")]:
...
def test_nan_group_key_rows_survive_company_aggregations():
    ...
    asset_npv = calculate_npv_per_asset(compute_yearly_npv_trajectories(_frame()))
    company_tech = aggregate_to_company_technology_npv(asset_npv)
    assert "CO_NAN_CO" in set(
        company_tech["company_id"]
