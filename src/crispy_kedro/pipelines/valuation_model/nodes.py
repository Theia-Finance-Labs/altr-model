...
def calculate_npv_per_asset(
    ...
    def get_discount_rate(scenario_type, scenario_name):
        ...
        if any(indicator in scenario_lower or indicator in type_lower for indicator in baseline_indicators):
    ...
    npv_data["discount_rate"] = npv_data.apply(
        lambda row: get_discount_rate(row.get("scenario_type", ""), row.get("scenario", "")), 
        axis=1
    )
    ...
    for asset_id, asset_data in npv_data.groupby("asset_id"):
        ...
        asset_data["discount_factor"] = (1 + discount_rate) ** (-asset_data["years_from_base"])
        ...
        if terminal_method == "perpetuity" and len(asset_data) > 0:
            ...
            if final_fcff > 0 and final_year < terminal_cutover_year:
                ...
                if discount_rate > terminal_growth_rate:
                    terminal_value_nominal = terminal_cf / (discount_rate - terminal_growth_rate)
                    
                    # Discount terminal value back to base year
                    years_to_terminal = final_year + 1 - base_year
                    terminal_discount_factor = (1 + discount_rate) ** (-years_to_terminal)
    ...
    logger.info(f"Baseline rate assets: {(npv_df['discount_rate'] == discount_rate_baseline).sum()}")
    logger.info(f"Shock rate assets: {(npv_df['discount_rate'] == discount_rate_shock).sum()}")
...
def aggregate_to_company_technology_npv(asset_npv: pd.DataFrame) -> pd.DataFrame:
    ...
    groupby_cols = ["company_id", "technology", "scenario_provider", "scenario", "scenario_type", "scenario_geography"]
    ...
    alignment_summary = asset_npv.groupby(groupby_cols).apply(
        lambda x: ", ".join(x["alignment_type"].unique())
    ).reset_index(name="alignment_type_mix")
    
    company_tech_npv = company_tech_npv.merge(alignment_summary, on=groupby_cols, how="left")
    
    logger.info(f"Aggregated to {len(company_tech_npv)} company-technology combinations")
...
def aggregate_to_company_npv(company_technology_npv: pd.DataFrame) -> pd.DataFrame:
    ...
    groupby_cols = ["company_id", "scenario_provider", "scenario", "scenario_type", "scenario_geography"]
    ...
    tech_mix = company_technology_npv.groupby(groupby_cols).apply(
        lambda x: ", ".join(sorted(x["technology"].unique()))
    ).reset_index(name="technology_mix")
    
    company_npv = company_npv.merge(tech_mix, on=groupby_cols, how="left")
    
    # Create sector mix summary  
    sector_mix = company_technology_npv.groupby(groupby_cols).apply(
        lambda x: ", ".join(sorted(x["sector"].unique()))
    ).reset_index(name="sector_mix")
    
    company_npv = company_npv.merge(sector_mix, on=groupby_cols, how="left")
    
    # Create alignment type summary
    alignment_mix = company_technology_npv.groupby(groupby_cols).apply(
        lambda x: ", ".join(sorted(set([item for sublist in x["alignment_type_mix"].str.split(", ") for item in sublist])))
    ).reset_index(name="alignment_type_mix")
